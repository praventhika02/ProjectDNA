...
export function FitLens({ score, readiness, breakdown, matched, missing, onFixGap }: FitLensProps) {
  const rows = [
  ...
  return (
    <section id="fit" className="scroll-mt-24">
      <div className="mb-4">
        <p className="font-mono text-xs uppercase tracking-[0.2em] text-cyan-200">Fit score</p>
        <h2 className="mt-2 text-2xl font-semibold text-slate-50 md:text-3xl">Why this role matches.</h2>
      </div>
      <div className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
        <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 text-center shadow-2xl shadow-cyan-950/20 backdrop-blur-xl">
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                {matched.slice(0, 4).map((item) => (
                  <div key={`${item.jobSkill}-${item.matchedRepoSkill}`} className="rounded-2xl border border-emerald-300/20 bg-emerald-300/10 p-3">
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                {missing.slice(0, 4).map((gap) => (
                  <div key={gap} className="rounded-2xl border border-amber-300/20 bg-amber-300/10 p-3">
                    <p className="text-sm text-slate-100">{gap}</p>
                    <button type="button" onClick={() => onFixGap(gap)} className="mt-2 font-mono text-xs text-cyan-200 hover:text-cyan-100">Fix this →</button>
                  </div>
                ))}
